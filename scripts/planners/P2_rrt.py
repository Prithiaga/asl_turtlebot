...
class RRT(object):
        ...
        for k in range(1, max_iters):
            ...
            if z <= goal_bias:
                ...
                #     np.random.uniform(self.statespace_lo[1], self.statespace_hi[1]))
                 x_rand = [np.random.randint(self.statespace_lo[0], self.statespace_hi[0]) for _ in range(len(self.x_goal))]
                #print("x_rand", x_rand)
            #print("V",V)
            x_near_index = self.find_nearest(V[0:n], x_rand)
            x_near = V[x_near_index]
            x_new = self.steer_towards(x_near, x_rand, eps)
            if self.is_free_motion(self.obstacles, x_near, x_new):
                #print("Is free motion")
                V[n,:] = np.array(x_new)
                P[n] = x_near_index
                n = n+1
                if np.array_equal(x_new, self.x_goal):
                    #reconstruct path
                    self.path = []
                    stateToAddToPath = self.x_goal
                    n = n-1
                    while True:
                        self.path.append(stateToAddToPath)
                        n = P[n]
                        if n == -1:
                            break
                        stateToAddToPath = V[n]
                    #reverse path
    ...
    def shortcut_path(self):
        ...
        """
        ########## Code starts here ##########
        allPathTraversed = False
        while not allPathTraversed:
            allPathTraversed = True
            for index, x in enumerate(self.path):
                if not np.array_equal(x, self.x_init) and not np.array_equal(x,self.x_goal) and self.is_free_motion(self.obstacles, self.path[index-1], self.path[index+1]):
                    self.path.pop(index)
                    allPathTraversed = False
        ########## Code ends here ##########
...
class GeometricRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        for index, item in enumerate(V):
            #print("Curr state ", item)
            currDist = np.linalg.norm(np.array(item) - np.array(x))
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        mag_x1_x2 = np.linalg.norm(x2_array - x1_array)
        if mag_x1_x2 < eps:
            return x2
        unit_vector_x1_x2 = (x2_array - x1_array) / mag_x1_x2
        # move/steer for eps distance from x1 along x1_x2_direction
        newpoint = x1 + (unit_vector_x1_x2 * eps)
        return newpoint
        ########## Code ends here ##########
...
class DubinsRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        min_index = 0
        for index, item in enumerate(V):
            d_path = dubins.shortest_path(item, x, self.turning_radius)
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        ########## Code starts here ##########
        path_x1_x2 = dubins.shortest_path(x1, x2, self.turning_radius * 1.0001)
        path_len = path_x1_x2.path_length()
        if path_len < eps:
            return x2
        samples = path_x1_x2.sample_many(eps)[0]
        #newpoint = samples[np.random.choice(len(samples))]
        newpoint = samples[1]
        return newpoint
        ########## Code ends here ##########
